			...
			try
			{
				...
				else
				{
					if (num != 1)
					{
						...
						var referenceHub = PlayerManager.players.Select(ReferenceHub.GetHub).FirstOrDefault(hub => hub.queryProcessor.PlayerId == component.owner.PlayerId);
						if (referenceHub == null)
						{
							GameCore.Console.AddDebugLog("SCPCTRL", "SCP-049 | Request 'finish recalling' rejected; no target found", MessageImportance.LessImportant);
							return false;
						}
						...
						{
							GameCore.Console.AddDebugLog("SCPCTRL", "SCP-049 | Request 'finish recalling' rejected; Debug code: ", MessageImportance.LessImportant);
					...
					if (component2 == null)
					{
						GameCore.Console.AddDebugLog("SCPCTRL", "SCP-049 | Request 'start recalling' rejected; provided object is not a dead body", MessageImportance.LessImportant);
						return false;
					}
					if (!component2.allowRecall)
					{
						GameCore.Console.AddDebugLog("SCPCTRL", "SCP-049 | Request 'start recalling' rejected; provided object can't be recalled", MessageImportance.LessImportant);
						return false;
					}
					var referenceHub2 = PlayerManager.players.Select(ReferenceHub.GetHub).FirstOrDefault(hub2 => hub2 != null && hub2.queryProcessor.PlayerId == component2.owner.PlayerId);
					if (referenceHub2 == null)
					{
						GameCore.Console.AddDebugLog("SCPCTRL", "SCP-049 | Request 'start recalling' rejected; target not found", MessageImportance.LessImportant);
						return false;
					}
					...
					GameCore.Console.AddDebugLog("SCPCTRL", "SCP-049 | Request 'start recalling' accepted", MessageImportance.LessImportant);
